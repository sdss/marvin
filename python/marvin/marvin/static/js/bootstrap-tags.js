        ...
        jQuery(function() {
            $.tags = function(element, options) {
                ...
                this.removeAllTags = function() {
                	_this.tagsArray.length = 0;
                	_this.popoverArray.length =0;
                	_this.renderTags();
                	return _this;
                };
                ...
                this.adjustInputPosition = function() {
                    ...
                    pWidth = _this.$element.actual('width') - pLeft;
                    oldWidth = _this.$element.width() - pLeft;
                    $(".tags-input", _this.$element).css({
                        paddingLeft: Math.max(pLeft, 0),
                        paddingTop: Math.max(pTop, 0),
                        width: pWidth
                    });
                    oldBottom = tagPosition != null ? tagPosition.top + tagElement.outerHeight(true) : 22;
                    pBottom = tagPosition != null ? tagPosition.top + tagElement.actual('outerHeight',{includeMargin: true}) : 22;
                    oldpadBot = oldBottom - _this.$element.height();
                    //console.log('cols','Left', 'pTop', 'Width', 'Bottom','padBot','width','height','outerWidth','outerHeight');
                    //console.log('old',oldLeft, pTop, oldWidth, oldBottom,oldpadBot,_this.$element.width(),_this.$element.height(),tagElement.outerWidth(true),tagElement.outerHeight(true) );
                    //console.log('new',pLeft, pTop, pWidth, pBottom,pBottom - _this.$element.actual('height'),_this.$element.actual('width'),_this.$element.actual('height'),tagElement.actual('outerWidth',{includeMargin:true}),tagElement.actual('outerHeight',{includeMargin:true}));
                    return _this.$element.css({
                        paddingBottom: pBottom - _this.$element.actual('height')
                    });
                };
