...
def results(query, request):
    ...
    query = Query(searchfilter=searchfilter, mode=query.mode, limit=10)
    r = query.run()
    yield r
    r = None
...
class TestResultsColumns(object):
    ...
    @pytest.mark.parametrize('cols, pars', [(['spaxelprop.x', 'spaxelprop.y', 'emline_gflux_ha_6564'],
                                             ['x', 'y', 'emline_gflux_ha_6564'])])
    def test_check_withadded(self, results, cols, pars, columns):
...
class TestResultsSort(object):
    ...
    def test_sort(self, results, data, limits):
        if results.mode == 'local':
            pytest.skip('skipping now due to weird issue with local results not same as remote results')
        results.sort('z')
        limit, count = limits
        assert data['resdict']['1'] == results.results[0]
        assert data['resdict'][str(count)] == results.results[count - 1]


class TestResultsPaging(object):

    @pytest.mark.parametrize('results', [('nsa.z < 0.1')], indirect=True)
    def test_check_counts(self, results, data, limits):
        if results.mode == 'local':
            pytest.skip('skipping now due to weird issue with local results not same as remote results')
        results.sort('z')
        limit, count = limits
    ...
    def test_get_next(self, results, data, chunk, rows, limits):
        if results.mode == 'local':
            pytest.skip('skipping now due to weird issue with local results not same as remote results')
        limit, count = limits
        results.sort('z')
        ...
        if results.count == results.totalcount:
            assert results.results[0] == data['resdict']['1']
            assert len(results.results) == count
        else:
            assert results.results[0] == data['resdict']['11']
            assert len(results.results) == chunk
            if rows:
                assert results.results[rows[0]] == data['resdict'][str(rows[1])]
    ...
    @pytest.mark.parametrize('index, chunk, rows',
                             ...
                             ids=['defaultchunk', 'chunk20'])
    def test_get_prev(self, results, data, index, chunk, rows, limits):
        if results.mode == 'local':
            pytest.skip('skipping now due to weird issue with local results not same as remote results')
        limit, count = limits
        results.sort('z')
        ...
        if results.count == results.totalcount:
            ...
            if rows:
                for row in rows:
                    assert results.results[row[0]] == data['resdict'][str(row[1])]
    ...
    @pytest.mark.parametrize('index, chunk, rows',
                             [(35, 10, [(0, 36)]),
                              (30, 20, [(0, 31), (15, 46)])],
                             ids=['defaultchunk', 'chunk20'])
    def test_get_set(self, results, data, index, chunk, rows, limits):
        if results.mode == 'local':
            pytest.skip('skipping now due to weird issue with local results not same as remote results')
        limit, count = limits
        results.sort('z')
