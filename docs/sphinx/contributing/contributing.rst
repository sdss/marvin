...
We define a global attribute, the ``name`` (required) of the VAC. `~.VACMixIn.get_data` is the only method that you need to override from `~.VACMixIn`. You will have noted that `~.VACMixIn.get_data` receives a single, ``parent_object`` argument, which is the object (e.g., a `~marvin.tools.cube.Cube`, `~marvin.tools.maps.Maps`, or `~marvin.tools.modelcube.ModelCube` object) that is trying to access the VAC information. You can use it and its attributes to do the matching with your VAC information.
